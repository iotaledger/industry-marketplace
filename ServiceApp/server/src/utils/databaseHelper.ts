...
const db = new sqlite3.Database(
    path.resolve(__dirname, database), async error => {
        ...
        await db.run('CREATE TABLE IF NOT EXISTS data (id TEXT PRIMARY KEY, deviceId TEXT, userId TEXT)');
    }
);
...
export const createSensorData = async ({ id, deviceId, userId }) => {
    await db.run('REPLACE INTO data (id, deviceId, userId) VALUES (?, ?, ?)', [id, deviceId, userId]);
};
